...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    const read = Effect.fn("CliServerDiscovery.read")(function* () {
      const raw = yield* Effect.tryPromise(() => readFile(file, "utf8")).pipe(Effect.orElseSucceed(() => undefined))
      if (!raw) return undefined
      try {
        return Option.getOrUndefined(decodeEntry(JSON.parse(raw)))
      } catch {
        return undefined
      }
    })
    ...
    const remove = Effect.fn("CliServerDiscovery.remove")(function* () {
      ...
      yield* Effect.tryPromise(() => rm(file, { force: true })).pipe(Effect.orElseSucceed(() => undefined))
    })
    ...
    return Service.of({
      write: Effect.fn("CliServerDiscovery.write")(function* (url) {
        yield* Effect.tryPromise(() =>
          writeFile(file, JSON.stringify({ url: localURL(url).toString(), pid: process.pid }), { mode: 0o600 }),
        ).pipe(Effect.orDie)
      }),
      remove,
